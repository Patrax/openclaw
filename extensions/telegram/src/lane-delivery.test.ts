...
function createHarness(params?: {
  ...
  const stopDraftLane = vi.fn().mockImplementation(async (lane: DraftLaneState) => {
    if (lane === lanes.answer && params?.answerMessageIdAfterStop !== undefined) {
      (
        answer as { setMessageId?: (value: number | undefined) => void } | undefined
      )?.setMessageId?.(params.answerMessageIdAfterStop);
    }
...
function expectStreamFinalized(
  result: LaneDeliveryResult,
): Extract<LaneDeliveryResult, { kind: "stream-finalized" }>["delivery"] {
  expect(result.kind).toBe("stream-finalized");
  if (result.kind !== "stream-finalized") {
    throw new Error(`expected stream-finalized, got ${result.kind}`);
  }
...
describe("createLaneTextDeliverer", () => {
  it("finalizes text-only replies in the active stream message", async () => {
    ...
    expect(expectStreamFinalized(result)).toEqual({ content: HELLO_FINAL, messageId: 999 });
  ...
  it("streams block and final text through the same lane", async () => {
    ...
    expect(blockResult.kind).toBe("stream-updated");
    expect(expectStreamFinalized(finalResult)).toEqual({ content: "done", messageId: 999 });
  ...
  it("falls back to normal delivery when no stream exists", async () => {
    ...
    expect(harness.sendPayload).toHaveBeenCalledWith({ text: HELLO_FINAL });
  ...
  it("streams the first long final chunk and sends follow-up chunks", async () => {
    ...
    expect(expectStreamFinalized(result)).toEqual({
